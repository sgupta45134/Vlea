...
class image_processor
{
     ...
     */
    public static function adjust_and_copy_file($tempfile, $newfilename, $systemcontext, $sectionid, $width, $height) {
        //require_capability('moodle/course:update', $coursecontext);

        $newfilename = str_replace(' ', '_', $newfilename);
        $storedfilerecord = self::stored_file_record($systemcontext->id, $sectionid, $newfilename);
        //print_object($storedfilerecord);die();
        $fs = get_file_storage();
        try {
            ...
            $tmproot = make_temp_directory('localrewardsimage');
            ...
            if (!empty($data)) {
                ...
                unset($tempfile);
                //print_object($newfile);die();
                return $newfile;
            } else {
                debugging('imagecannotbeused', 'format_collapsibletopics', DEBUG_DEVELOPER);
            }
        } catch (Exception $e) {
            if (isset($tempfile)) {
                unset($tempfile);
    ...
    private static function process_image($filepath, $requestedwidth, $requestedheight, $mime) {
        ...
        imagecopy($finalimage, $original, 0, 0, 0, 0, 0,0);
