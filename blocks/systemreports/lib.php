...
function get_lpt_id($l_id, $t_id) {
    global $DB;
    $lpt_id = $DB->get_record_sql('select id  from {systemreports_training}  where lp_id=? AND t_id=?', array($l_id, $t_id));
    $lpt_id = $lpt_id->id;
    return $lpt_id;
}
...
function learningplan_training($lp_id) {
    global $DB;
    $training = $DB->get_records('systemreports_training', array('lp_id' => $lp_id));
    return $training;
}
...
function user_learningplan($u_id) {
    global $DB;
    $training = $DB->get_recordset_sql('SELECT lp_id as id, (select  systemreports  from {learning_learningplan} where id =lp_id) as learningplan
...
function status_value($status_id) {
    $status_value = '';
    if ($status_id == '0') {
        $status_value = get_string('status_in_progress', 'block_systemreports');
    } else if ($status_id == '1') {
        $status_value = get_string('status_not_started', 'block_systemreports');
    } else if ($status_id == '2') {
        $status_value = get_string('status_completed', 'block_systemreports');
    }
    return $status_value;
}
...
function status_id($l_id, $u_id, $t_id) {
    global $DB;
    $status = $DB->get_record_sql('select lut.id as id from {systemreports_training} lpt inner join {learning_user_trainingplan} lut
...
function get_learningplan_name($id) {
    global $DB;
    $result = $DB->get_record_sql("SELECT systemreports FROM {learning_learningplan} WHERE id = ?", array($id));
    return $result->systemreports;
}
...
function delete_learningplan_record($table, $id, $url, $lp_id = '') {
    ...
    if ($table == 'learning_learningplan') {
        ...
        $DB->delete_records('systemreports_training ', array('lp_id' => $id));
        ...
        $DB->delete_records('systemreports_training', array('t_id' => $id));
    } else if ($table == 'systemreports_training') {
        $DB->delete_records('systemreports_training', array('id' => $id));
    } else if ($table == 'learning_rolebased_lp') {
        // $DB->delete_records('learning_user_learningplan', array('id'=> $id));
        $DB->delete_records('learning_rolebased_lp', array('id' => $id, 'lp_id' => $lp_id));
    }
    redirect($url, get_string('removed', 'block_systemreports'), 2);
}
...
function display_list($lp_id, $u_id) {
    ...
    $table->id = 'statuslist';
    $table->head = array(get_string('s_no', 'block_systemreports'), get_string('training_name', 'block_systemreports'), get_string('training_method', 'block_systemreports'), get_string('start_date', 'block_systemreports'), get_string('end_date', 'block_systemreports'), get_string('status', 'block_systemreports'), get_string('remarks', 'block_systemreports'));
    ...
    $sql = 'select  t_id, (select type_id from  {learning_training} where id =t_id) as type_id, lp_id, lut.status, lut.remarks, `u_id` as id,(select training_name from  {learning_training} where id =t_id)
            as training, (select url from {learning_training} where id =t_id) as url, (select systemreports from   {learning_learningplan} where id =lp_id) as systemreports, (SELECT
            ...
            where id =t_id)as date2 from {systemreports_training} lpt inner join {learning_user_trainingplan} lut
    ...
    if ($DB->record_exists_sql($sql, array($lp_id, $u_id))) {
        foreach ($rs as $log) {
            ...
            $row[] = ++$inc;
            if (strlen($log->url) > 0) {
                $row[] = '<a title="' . get_string('training', 'block_systemreports') . '" href="' . $log->url . '">' . format_string($log->training, false) . '</a>';
            } else {
                $row[] = format_string($log->training, false);
            }
            if ($log->type_id == 1) {
                $training_method = get_string('elearning', 'block_systemreports');
            } else if ($log->type_id == 2) {
                $training_method = get_string('classroom', 'block_systemreports');
            } else if ($log->type_id == 3) {
                $training_method = get_string('onthejob', 'block_systemreports');
            }
...
function training_type($type_id) {
    $training_type = '';
    if ($type_id == '1') {
        $training_type = get_string('elearning', 'block_systemreports');
    } else if ($type_id == '2') {
        $training_type = get_string('classroom', 'block_systemreports');
    } else if ($type_id == '3') {
        $training_type = get_string('onthejob', 'block_systemreports');
    }
    return $training_type;
}

/**
 * This function return the learning stages.
 *
 * @param int $learning_id learning id of stage
 * @return string learning stage name.
 */
function learning_stage($learning_id) {
    $learning_stage = '';
    if ($learning_id == '1') {
        $learning_stage = get_string('learninig_plan_stage1', 'block_systemreports');
    } else if ($learning_id == '2') {
        $learning_stage = get_string('learninig_plan_stage2', 'block_systemreports');
    } else if ($learning_id == '3') {
        $learning_stage = get_string('learninig_plan_stage3', 'block_systemreports');
    } else if ($learning_id == '4') {
        $learning_stage = get_string('learninig_plan_stage4', 'block_systemreports');
    }
...
function lp_send_notification($user, $msg, $table, $subject) {
    ...
    $message->component = 'block_systemreports';
    $message->name = 'view';
...
function get_lp_training($id) {
    ...
    $training_sql = "SELECT lt.training_name, lt.type_id, lt.start_date, lt.end_date
					FROM {systemreports_training} lpt
    ...
    if ($DB->record_exists_sql($training_sql, array($id))) {
        ...
        $table->data[] = array('', '', get_string('notfound', 'block_advance_systemreports'), '', '');
    }
...
function get_cohorts() {
    ...
    if(!empty($cohorts)) {
        foreach($cohorts as $cohort) {
            $return[$cohort->id] = $cohort->name;
        }
    }
    return $return;
}

function get_course_order($courseid, $lpid) {

    global $DB;

    $rec = $DB->get_record('systemreports_courses', array('course'=>$courseid, 'learningplanid'=>$lpid));
    return $rec;
}

function get_role_string($roleid) {
    global $DB;

    $profile = $DB->get_record('profile', array('id'=>$roleid), 'id, name');
    if(!empty($profile)) {
        return $profile->name;
    } else {
       return 'NA';
